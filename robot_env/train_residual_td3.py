...
def main(cfg: ResidualTD3DexmgConfig):
    ...
    # )
    policy_dir = Path("/mnt/data/wql/residual-offpolicy-rl/artifacts/run_4yon5tmg_model_step_199999:v0/policy")

    base_policy: ACTPolicy = load_policy(policy_dir)

    print("This is a base_policy:",base_policy)
    ...
    if cfg.algo.offline_fraction == 1.0:
        print("Offline-only training mode: offline_fraction=1.0")
    ...
    def _populate_offline_buffer(
        ...
        if use_base_policy_for_base_actions:
            print("Using base policy to generate base actions for offline transitions")
        ...
        # The most important part: iterate through dataset sequentially and create transitions
        for sample in tqdm(loader, desc="Processing offline dataset"):
            ...
            if ep_idx in episode_cache:
                ...
                # print(sample["next.done"], float(done_flag))
    ...
    def _run_critic_warmup(
        ...
        for i in range(cfg.algo.critic_warmup_steps):
            ...
            with training_timer.time("batch_sampling"):
                ...
                if cfg.algo.offline_fraction > 0.0:
                    print(f"Warmup step with offline data")
                    ...
                    # batch = offline_batch  # Use only offline batch for warmup to focus on learning from dataset
                else:
                    # Online-only training
                    # batch = online_batch
                    print(f"This warmup step may not be meaningful since offline_fraction=0.0 and online buffer is already filled with random data")
