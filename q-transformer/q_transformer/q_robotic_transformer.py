...
class QRoboticTransformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        text_model_path: str | None = None,    # 本地 T5 模型目录；None 则使用 HuggingFace Hub 默认下载
        ...
        dual_critics = False,
        num_cameras = 1,                        # number of camera views; >1 enables multi-view encoding
    ):
        super().__init__()
        ...
        assert num_cameras >= 1, 'num_cameras must be >= 1'
        ...
        if condition_on_text:
            ...
            # 若指定了本地模型路径，则通过 model_names 传给 conditioner，
            # 优先级高于 conditioner_kwargs 中已有的 model_names 设置。
            merged_conditioner_kwargs = dict(conditioner_kwargs)
            if text_model_path is not None and 'model_names' not in merged_conditioner_kwargs:
                merged_conditioner_kwargs['model_names'] = text_model_path
    ...
    def encode_state(
        ...
        einops
        b - batch
        c - channels (3 for single view, V*3 for multi-view)
        ...
        # ------------------------------------------------------------------
        # Multi-view support
        # When num_cameras > 1, video is expected to have shape
        # (b, V*3, f, h, w) where the V camera views are channel-stacked.
        # We split into (b, V, 3, f, h, w), rearrange to (b, f, V, 3, h, w),
        # and flatten into (b*f*V, 3, h, w) so that the shared ViT backbone
        # processes every (frame, view) pair independently.
        # ------------------------------------------------------------------
        ...
        if num_cameras > 1:
            # video: (b, V*3, f, h, w)
            video = rearrange(video, 'b (v c) f h w -> b f v c h w', v = num_cameras, c = 3)
            # (b, f, V, 3, h, w) → flatten (b, f, V) into batch
        ...
        cond_fns, _ = self.conditioner(
            **text_cond_kwargs,
            cond_drop_prob = cond_drop_prob,
            repeat_batch = (*((frames * num_cameras,) * self.num_vit_stages), *((1,) * self.transformer_depth * 2))
        )
        ...
        # Single-view: (b, f, c, h, w)
        # Multi-view:  (b, f, V, c, h, w)

        learned_tokens = self.token_learner(tokens)
        # Single-view: (b, f, c, n)
        # Multi-view:  (b, f, V, c, n)

        # tokens_per_frame = num_learned_tokens * num_cameras
        tokens_per_frame = self.num_learned_tokens * num_cameras

        if num_cameras > 1:
            learned_tokens = rearrange(learned_tokens, 'b f v c n -> b (f v n) c')
        else:
            learned_tokens = rearrange(learned_tokens, 'b f c n -> b (f n) c')

        # causal attention mask
        # Causality is enforced at the frame level; all tokens within the same
        # frame (including all views) can attend to each other freely.

        attn_mask = ~torch.ones((frames, frames), dtype = torch.bool, device = device).triu(1)
        attn_mask = repeat(attn_mask, 'i j -> (i r1) (j r2)', r1 = tokens_per_frame, r2 = tokens_per_frame)

        # sinusoidal positional embedding (per-frame; shared across views and tokens within each frame)

        pos_emb = posemb_sincos_1d(frames, learned_tokens.shape[-1], dtype = learned_tokens.dtype, device = learned_tokens.device)

        learned_tokens = learned_tokens + repeat(pos_emb, 'n d -> (n r) d', r = tokens_per_frame)
