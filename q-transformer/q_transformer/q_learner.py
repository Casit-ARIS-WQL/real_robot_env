...
class QLearner(Module):
    ...
    def __init__(
        ...
        *,
        ...
        learning_rate: float,
        min_reward: float = 0.,
        reward_scale: float = 1.,   # divide raw rewards by this value to keep Q targets in [0, 1].
                                    # For sub-episode sparse rewards labeled 1-6 (one reward per sub-episode,
                                    # zero for all other steps), set this to 21.0 (= 1+2+3+4+5+6, the maximum
                                    # possible undiscounted cumulative reward across all 6 sub-episodes).
                                    # Important: set done=True ONLY at the very last timestep of the full task
                                    # (after sub-episode 6 completes), not at sub-episode boundaries.
    ...
    def n_step_q_learn(
        ...
        *,
        ...
        rewards, _ = pack([rewards / self.reward_scale, q_next_value], 'b *')
    ...
    def autoregressive_q_learn(
        ...
        *,
        ...
        # text_embeds may arrive as (b, d) when the dataset provides one embedding
        # per episode rather than per timestep. Repeat across timesteps so the batch
        # dimension matches the packed states batch (b*t).
        if text_embeds.shape[0] != states.shape[0]:
            text_embeds = repeat(text_embeds, 'b ... -> (b r) ...', r = num_timesteps)
        ...
        if self.use_bce_loss:
            ...
            losses_last_action = F.mse_loss(q_pred_last_action.sigmoid(), q_target_last_action_value, reduction = 'none')
    ...
    def learn(
        ...
        *args,
        ...
        #
        # json_dataloader / MockReplayDataset (6-item):
        #   (texts_or_embeds, states, actions, next_states, rewards, dones)
        #   where texts_or_embeds is either list[str] or a pre-computed tensor.
        #
        # ReplayMemoryDataset (7-item):
        #   (text_embeds, states, actions, next_states,
        #    next_text_embed, rewards, dones)  — all tensors.
        # ------------------------------------------------------------------

        texts_or_embeds = args[0]

        # Embed raw instruction strings when provided
        if (
            isinstance(texts_or_embeds, (list, tuple))
            and len(texts_or_embeds) > 0
            and isinstance(texts_or_embeds[0], str)
        ):
            model = self.unwrap(self.model)
            text_embeds = (
                model.embed_texts(list(texts_or_embeds))
                if model.condition_on_text
                else None
            )
            args = (text_embeds, *args[1:])

        # For n-step / multi-action methods a separate next_text_embed is
        # required (7-item format).  When only 6 items are present, reuse
        # the current text embedding — the task instruction is constant
        # across an episode so next_text_embed == text_embeds.
        needs_next_embed = self.n_step_q_learning or self.is_multiple_actions
        if needs_next_embed and len(args) == 6:
            text_embeds, states, actions_arg, next_states, rewards, dones = args
            args = (text_embeds, states, actions_arg, next_states,
                    text_embeds, rewards, dones)
        ...
        conservative_reg_loss = ((q_actions_not_taken - ((min_reward * num_timesteps) / self.reward_scale)) ** 2).sum() / num_non_dataset_actions
